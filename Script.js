...
const subtitleSmall = document.querySelector('.help-subtitle-small'); // заголовок "Чем ты полезен?"
let activeCard = null;
let wordTimer = null;
let glowTimer = null;
...
function showDetails(category, type) {
    ...
    content.items.forEach(item => {
        // Вставляем и изображение, и иконку (иконка скрыта по умолчанию)
        html += `
    ...
    images.forEach(img => {
        img.onerror = function() {
            this.style.display = 'none'; // скрываем битое изображение
            const icon = this.nextElementSibling; // иконка (сразу после img)
...
cards.forEach(card => {
    card.addEventListener('click', function() {
        ...
        if (this.classList.contains('active') && descriptionDiv.classList.contains('visible')) {
            ...
            hideExtraButtons();
            hideDetails();
            // Скрываем заголовок "Чем ты полезен?"
            ...
            // Показываем заголовок "Чем ты полезен?"
